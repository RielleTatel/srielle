...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
    ...
    uv.y*=-1.;
    // Deviation from upstream: slow horizontal scroll only — calm ribbon-flow that
    // matches the reference. Vertical bob removed. Warp + CPPN morph kept gentle so
    // the bands read as sweeping streamers rather than churning blobs.
    float scroll=uTime*0.18;
    uv.x=mod(uv.x+scroll+2.0,4.0)-2.0;
    uv+=uWarp*vec2(sin(uv.y*6.283+uTime*0.3),cos(uv.x*6.283+uTime*0.3))*0.05;
...
void main(){
    ...
    vec3 colorTinted=intensity*uColor;
    col.rgb=mix(col.rgb,colorTinted,uUseColor);

    float scanline_val=sin(gl_FragCoord.y*uScanFreq)*0.5+0.5;
    col.rgb*=1.-(scanline_val*scanline_val)*uScan;
    col.rgb+=(rand(gl_FragCoord.xy+uTime)-0.5)*uNoise;
    // Deviation from upstream: intensity-based alpha so the page bg shows through.
    // smoothstep gives sharper plasma silhouettes than a linear ramp.
    float a=(col.r+col.g+col.b)/3.0;
    a=smoothstep(0.02,0.50,a);
    gl_FragColor=vec4(clamp(col.rgb,0.0,1.0),clamp(a,0.0,1.0));
}`;
...
export default function NoirFlow({
  color = "#1a6fd4",
  speed = 0.4,
  warpAmount = 0.15,
  hueShift = 0,
  ...
  // Mutable mirror so the render loop reads the latest props without re-creating the WebGL context.
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      lastT = now;
      simT += dt * propsRef.current.speed;
      ...
      program.uniforms.uWarp.value = propsRef.current.warpAmount;
